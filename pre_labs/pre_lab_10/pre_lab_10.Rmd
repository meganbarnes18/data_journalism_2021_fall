...
https://datawrapper.dwcdn.net/8J8jK/1/
...
The code created a dataframe for the count of county level approved loan applications. The output is a table that shows the county name and count loans for each. 
```{r}
...
**Answer** Describe what the code below is doing and the output.
The code below filters out the non-maryland ppp loan applications, as the ones that don't belong have 6 loans or less. The output shows that 8 counties from the previous codeblock did not belong there. We now have strictly Maryland counties in our dataframe. 
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
The new column gives us the 5-digit FIPS codes for every county in Maryland. 
```{r}
...
I see 8 columns with the same labels as both of our previous tables. We still have the county names in all caps, but we still see the changes we made in the 'county' column. We have the same FIPS codes as well, but now everything is in one place. 
```{r}
...
Before, we had a map of all counties in the US, and all except those in Maryland were gray. Now, we have just Maryland and all of the counties are in different colors. 
...
**Answer** Describe the changes from the previous map
Now, we have the names of counties on the map as opposed to just their FIPS codes. We also have commas in the numbers, making them easier to read and analyze.
...
https://datawrapper.dwcdn.net/SSesW/1/