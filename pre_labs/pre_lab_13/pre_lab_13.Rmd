...
**Answer** We get a dataframe with the same five fields listed above
```{r}
...
**Answer** There are more dots located in places where Trump received a higher percentage. It's not a perfect representation, but I can see a trend. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** There doesn't seem to be a relationship here, as the dots are really spread out. 
```{r}
...
**Answer** I see a table that shows the relationships between the variables above. The relationship between both sets is positive, but ppe_amount_per_employee and pct_trump isn't as strong. 
```{r}
reveal_data %>%
...
**Answer** We get a more clunky output that shows us the p-value. We see here that it's very low. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** We have a higher p-value here, but it's still not a strong value. 
```{r}
...
**Answer** I see a table that compares Clinton and Trump, specifically how their votes affect mean_ppp_amount_per_employee and mean_businesses_receiving_ppe_pct. 
...
**Answer** I see the two means along with their t and p-values. These values are pretty high, so we can determine that these numbers are significant. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** I see the business means and their p and t-values. These values suggest a pattern. 
```{r}
