...
**Answer** First, we store the data in r as maryland_ppp_top_counties. This ensures that we can manipulate the data through filter, group_by, etc.. Next, we group the data by project_county_name, as this is the column with the county data. Then, we use summarise to break-down our data into a single category, which, in this case, is total_loans. We use the n() function to count the number of records in this category. Finally, we use the arrange function to put our results in descending order, and we can see which county had the most total loans. "Head" limits our results to the first 10 counties. The last line of code displays the data so we can check our work and see what we get. In this case, we'll see a table with two columns: project_county_name on the left and total_loans on the right.
...
**Answer** I see nothing but a big gray box. 
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** Now I see a bar chart with our 10 counties.It's a little messy though.
...
**Answer** The order of the counties changed from alphabetical order to ascending order.The names still overlap.  
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** We can clearly see the county names now, and the chart is very easy to read. We can see that Montgomery has the most loans and Carroll has the least. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** This codeblock applied a minimal theme to the bar chart. 
...
**Answer** The background of the chart is now blue. 
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The code added a title to the chart as well and the x and y axis. It also added a caption that gives the source of the data.  
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see a table with two columns: date_approved on the left and total_loans on the right. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see a line chart that shows a decrease in loans over time. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** I see the same chart, this time with more concise/specific dates. It's still very messy.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The date labels are more spread out and easier to read. We can now analyze the data based on the chart. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
