...
**Answer** There were no parsing errors shown when I ran the code.
...
**Answer**: 192,959 loans are NA, and Subway has 275 loans.
...
**Answer**: There were 135,073 loans, whuch is the same number as forgiveness_amount.
...
**Answer**: 135, 073 loans. This is the same amount as forgiveness_date and forgiveness_amount, leading to the conclusion that these will always be the same. 
```{r}
ppp_maryland_loans %>%
...
**Answer** There are no laons in the data for October 2020.
```{r}
ppp_maryland_loans %>%
...
**Answer** $10,000,000	is the largest loan and $6 is the smallest. This is a little weird because a $6 loan seems very small. 
```{r}
...
**Answer** 1. The first and second rows are exactly the same. 2. The city field has misspellings of Arnold, so grouping and counting would be difficult. 3. "The amount column is stored as a character, not a number" (ch. 11). You can't do any math to it. 
```{r}
arnold_md_loans
```
...
**Answer** There's now an 'x' in front of it, making it "x1_id"
...
**Answer** Error: Problem with `summarise()` column `total_amount`. ℹ `total_amount = sum(amount)`. x invalid 'type' (character) of argument; This might mean that the data type is wrong and we can't use math here. 
